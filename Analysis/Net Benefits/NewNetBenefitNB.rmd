...
generateStatic = function(baseParameters, strategyParameters, shockParameters, title) {
  ...
  spray_segment =
    spraydf %>%
      filter(t <= baseParameters[["numYears"]] * 365, numInvasions == baseParameters[["numInvasions"]], 
  ...
  noaction_segment = 
    df %>%
    filter(strategy_names == "NoAction", t <= baseParameters[["numYears"]] * 365, numInvasions == baseParameters[["numInvasions"]]) %>%
    group_by(t,strategy_names) %>%
    summarize(returns = mean(returns)) %>%
    mutate(yearlyReturns = returns/(t/365),
           anAcreReturns = yearlyReturns / baseParameters[["numAcres"]],
  ...
  basedf = basedf %>% select(t, anAcreReturns, strategy_names, id, shock)
  ...
  shockdf = data.frame(t=numeric(), anAcreReturns=numeric(), id=character(), shock=numeric())
  ...
  if (max(c("efficacy") %in% keys(shockParameters)) || baseChange) {
    ...
    spraydf %>%
      ...
      mutate(yearlyReturns = returns/(t/365),
             ...
             id="S[2]",
             shock=TRUE) %>%
      select(t, anAcreReturns, strategy_names, id, shock)
  ...
  static = 
    alldf %>%  
      ggplot() +
      geom_line(size=1,aes(x=t/365,y=anAcreReturns, color=as.factor(strategy_names), linetype=shock)) +
      theme_classic() +
...
strategyParams[["efficacy"]] = 0.6
strategyParams[["frequency"]] = 6
strategyParams[["radius"]] = 20
shockParams[["numInvasions"]] = 25
