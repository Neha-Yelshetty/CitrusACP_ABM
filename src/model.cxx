...
#include "../headers/biologicalmodel.hpp"
...
void InitialiseCHMA(Commodity crop) {
    ...
    int cropRowSize = latticeRows ;
    int cropColSize = latticeCols ;
    int k =0;
    cout << "gridLength : " << ParameterSet::gridLength << " gridWidth : " << ParameterSet::gridWidth <<
    " latticeRows: " << latticeRows << " latticeCols : " << latticeCols << " cropRowSize: " << cropRowSize <<
    " cropColSize: " << cropColSize << endl;
    for (int i = 0; i < ParameterSet::gridLength*ParameterSet::gridWidth; i++) {
        //Create grove

        int i_lb = 0 ; //(i / ParameterSet::gridLength) * cropRowSize;
        int i_ub = 157; //i_lb + cropRowSize;
        int j_lb = 0;//(i % ParameterSet::gridLength) * cropColSize;
        int j_ub = 69;//j_lb + cropColSize;
        cout << " i : " << i << " i_lb : " << i_lb << " i_ub : " << i_ub << " j_lb : " << j_lb << " j_ub : " << j_ub << endl;
        bool agency = (stoi(agencyParams[0]) == 1);
       
        agents[i] = Grove(crop, agency, i_lb, i_ub, j_lb, j_ub,i);
        
        //Create and assign strategy vectors
...
************************************************************/
void Phase2(int farmid) {
    int period_t = bioABM::getModelDay();
    int relativePeriod = period_t % 365;
    for (int k = 0; k < agents[farmid].behaviorPatterns.size(); k++) {
        if (agents[farmid].behaviorPatterns[k]->actionPlannedOnDay(relativePeriod)) {
            agents[farmid].behaviorPatterns[k]->executeAction(&agents[farmid]);
        }
...
double getMeanHLB(Grove g,int farmid) {
    int* ibounds = agents[farmid].getIBounds();
    int* jbounds = agents[farmid].getJBounds();
...
void Phase4(int farmid) {
    ...
    if ( (period_t % 365) != 0 ) { return; }
        for (int k = 0; k < agents[farmid].behaviorPatterns.size(); k++) {
            agents[farmid].behaviorPatterns[k]->PlanActions();
        }
...
void Phase5(int farmid) {
    ...
    int rel_t = t % 365;
        ...
        if (rel_t == 0) {
            ...
            agents[farmid].costs += (agents[farmid].getCrop()->costs) * (10833) ; //36.05 * (10833) 
            
        }

        //Harvest
        if (agents[farmid].getCrop()->isHarvestPeriod(rel_t)) {
            for (int k = ibounds[0]; k < ibounds[1]; k++) {
                for (int l = jbounds[0]; l < jbounds[1]; l++) {
                    ...
                    agents[farmid].returns += adjustedReturns;
...
* Write CSV Line
***************************************************************/
void writeCSVLine(int farmid) {
        double meanSeverity = getMeanHLB(agents[farmid],farmid);
        stringstream strategyNames;
        stringstream strategyParams;
        stringstream rougetreeremoved;
        if (agents[farmid].behaviorPatterns.empty()) {
            strategyNames << "NoAction";
            strategyParams << "0;0;0;0;0;0;0";
        }
        else {
            for (int k = 0; k < agents[farmid].behaviorPatterns.size(); k++) {
                strategyNames << agents[farmid].behaviorPatterns[k]->getName();
                strategyParams << agents[farmid].behaviorPatterns[k]->getParams();
                rougetreeremoved << agents[farmid].behaviorPatterns[k]->getRougeTreeRemovalount();
                if (k != agents[farmid].behaviorPatterns.size() - 1) {
                    strategyNames << "-";
                    strategyParams << "-";
                    rougetreeremoved << "-";
                }
        ...
        outputFile << farmid << ",";
        outputFile << agents[farmid].costs << ",";
        
        outputFile << agents[farmid].returns << ",";
        outputFile << (agents[farmid].returns - agents[farmid].costs) << ",";
